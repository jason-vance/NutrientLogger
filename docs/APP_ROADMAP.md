...
- [x] **`ConsumedMealsView`: one card per meal** — collapsed each meal into a single `ConsumedMealCard` with an aggregated calorie/macro summary (`CircleChart` rings, same visual language as the per-food version) and foods listed as simple rows inside the card. Kept the delete-whole-meal action as the card header. Per-meal totals reuse the already-tested `NutrientDataAggregator` (fed the meal's fetched `FoodItem`s), so no new pure aggregation math needed dedicated tests.
- [x] **Add-food button per meal** — added to the bottom of each meal card, deep-linking to `FoodSearchView` pre-filled with that meal's date and time. Required threading new `initialDate`/`initialMealTime` params through `FoodSearchView` → `FoodDetailsView` (defaulting to today / `.forCurrentTime()`, unchanged for every other existing call site); logged-food edits still override via the existing `populateFieldsIfNeeded()`.
- [x] **`ConsumedNutrientDetailsView`: one card per meal** — same per-meal card treatment for "Contributing Foods", replacing per-food cards grouped under a plain meal-time label. `ConsumedNutrientDetailsFoodRow` gained a `wrapInCard` flag so it can render as a plain row nested inside the new meal card (unchanged/still self-carded for its other caller, `ConsumedWaterView`).
- [x] **Fix duplicate-foods bug** — root cause wasn't the `ConsumedNutrientDetailsView` ForEach itself (already keyed by array offset) but two upstream `Identifiable` conformances that were too coarse: `FoodItem.id` was hardcoded to `-1` for every food (fixed: now `fdcId`), and `NutrientFoodPair.id` was just `"nutrientNumber-fdcId"`, which collides for two separate real log entries of the same food (fixed: now also includes date/meal/portion/amount). Added a defensive dedup in `ConsumedNutrientDetailsView.mealFoods` against exact-value-duplicate `FoodItem`s as a second layer of protection.

**Behavioral Design** *(mostly copy/reorder, low code risk)*
- [x] **Goal gradient effect** — gave each "0% at day start" moment a small visual floor so it never renders as a totally blank/dead state (the actual numbers shown stay accurate — this is decorative only):
  - `CalorieGoalRing`: minimum 2% arc.
  - `WaterWaveCircle` (`ConsumedWaterView`): minimum 3% wave fill.
  - `StreakCardView`: no longer hides itself entirely at count 0 (was `if count > 0`) — now shows a dimmed flame + "Start a Day/Week Streak" invitation, with the mini milestone progress bar given the same minimum-fill treatment.
- [x] **Loss aversion** — `MarketingView` now shows a contextual "Don't miss out on X" headline (matching the existing "Don't break your streak!" notification framing) for paywall triggers tied to one specific locked feature (trend charts, Health sync, weight goals, micronutrient goals, CSV export, weekly nutrient watch); generic entry points (smart paywall, deep link, profile upsell) keep the neutral headline.
  - Correction to this doc: there is no lifetime/family tier in the code — the v4.0 "consider adding a third tier" checkbox was apparently decided against, not shipped. The "trial-ending copy" idea also has no existing surface (no trial-ending reminder flow exists) — scoped this item to the achievable part instead.
- [x] **Contrast effect** — re-scoped for the reason above: instead of anchoring against a lifetime/family tier, `MarketingView`'s yearly button is now the bold filled "primary" CTA with a monthly-equivalent price ("Just $X.XX/mo") next to its lump sum, while monthly is a plain outlined "secondary" button — the same $90-steak-next-to-a-$40-salmon anchor, applied to the two tiers that actually exist. (`OnboardingPaywallView` already had an equivalent "Best Value" treatment from v4.0; this brings the general in-app paywall up to the same standard.)
- [x] **Endowment / IKEA effect** — updated footer copy on `NutritionSettingsSheet` and `BodyMetricCustomizeSheet` ("build your Nutrition/Body tab exactly the way you want it") and `CustomFoodsLibraryView`'s title/empty-state ("My Custom Foods", "saved to your own personal library").
...
- [x] **Add demo dashboard to onboarding** — a pre-populated sample day shown after personalization (before the paywall) so the user sees the end-state they're working toward.
...
- [x] **Let existing/returning users personalize too** — a "Personalize → Diet & Focus" entry point at the top of Nutrition Settings pushes `NutritionPersonalizationSettingsView`, which loads the persisted `User.dietPreset` / `User.nutritionConcern`, shows both pickers with live reflect-back lines, and re-applies the ordering. Non-clobbering apply by default (fills only groups the user hasn't hand-arranged); when a custom arrangement exists it also offers an explicit "Reset to Suggested Order" (confirmed) that re-sorts every promoted group. Reuses `NutritionPersonalizationApplier`; logs `settings_personalization_updated`.
